    ...
    def ConstantRefresh(s): #Jumping and score
        ...
        if s.cont == "Y":
            ...
            for i in range(len(s.hazard)):
                ...
                for x in range(2):
                    tempx = x * 2
                    if hazardposition[tempx] >= dinoposition[0] and hazardposition[tempx] <= dinoposition[2] and hazardposition[tempx + 1] >= dinoposition[1] and hazardposition[tempx + 1] <= dinoposition[3]:
                        s.cont = "N"
                ...
                if choice != "Nothing":
                    if choice == "Small" or choice == "Large":
                        ...
                        for i in range(amount):
                            s.hazard.append("")
                            s.hazard[len(s.hazard) - 1] = s.canvas.create_rectangle(vertices[0] + (gap * i), vertices[1], vertices[2] + (gap * i), vertices[3], fill = "Black")

                    elif choice == "Birds":
                        position = (amount - 1) * 100
                        vertices = [1900, 240 + position , 1950, 300 + position]
                        s.hazard.append("")
                        s.hazard[len(s.hazard) - 1] = s.canvas.create_rectangle(vertices[0], vertices[1], vertices[2], vertices[3], fill = "Blue")
